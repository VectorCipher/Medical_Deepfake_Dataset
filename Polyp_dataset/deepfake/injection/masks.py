...
masks.py
========
Loads your real binary masks (preferred) and falls back to a bbox-derived
rectangle only if a mask file is missing for a given image. Also handles
...
def bbox_fallback_mask(img_shape, x1, y1, x2, y2, pad=8):
...
def prepare_initial_mask(filename, img, bbox=None):
    """
    ...
    if mask is None:
        ...
        mask = bbox_fallback_mask(img.shape, *bbox)
