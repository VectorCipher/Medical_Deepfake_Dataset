...
def refine_and_inpaint(filename, engines: dict, critic, bboxes=None):
    """
    engines: dict like {"lama": LamaEngine_instance, "sd": SDInpaintEngine_instance}
    bboxes: optional list of (x_min, y_min, x_max, y_max) tuples, used only if a real mask file is missing
    """
    ...
    mask, mask_source = prepare_initial_mask(filename, img, bboxes=bboxes)
    ...
    for it in range(1, MAX_ITERS + 1):
        ...
        # escalate for next attempt by dilating the mask
